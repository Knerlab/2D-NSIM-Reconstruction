# -*- coding: utf-8 -*-
"""
Created on Fri Aug 16 14:45:07 2024
...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 17 15:31:24 2023
...
class si2D(object):
    ...
    def sepmatrix(self):                                             # for 20 px 7phs, 1 phs shift 1px
        nphases = self.nph
        ...
        for j in range(nphases):
            ...
            for order in range(1, norders):
                sepmat[2 * order - 1, j] = 2 * np.cos( 2 * np.pi * (j * order) / (nphases+1)) / (nphases+1)
                sepmat[2 * order, j] = 2 * np.sin(2 * np.pi * (j * order) / (nphases+1) ) / (nphases+1)
    ...
    def separate(self, nangle):                       #7phs 7order
        angle, nph, Nw, Nw = self.img.shape
        ...
        self.separr[5] = np.fft.fftshift(self.interp(((outr[5] + 1j * outr[6])/2).reshape(Nw, Nw)) * self.winf)
        self.separr[6] = np.fft.fftshift(self.interp(((outr[5] - 1j * outr[6])/2).reshape(Nw, Nw)) * self.winf)
        self.separr[7] = np.fft.fftshift(self.interp(((outr[7] + 1j * outr[8])/2).reshape(Nw, Nw)) * self.winf)
        self.separr[8] = np.fft.fftshift(self.interp(((outr[7] - 1j * outr[8])/2).reshape(Nw, Nw)) * self.winf)
    ...
    def shift3(self, angle, spacing):
      ...
      ysh[1, :, :] = np.exp(2j * np.pi * (-kx * self.xv - ky * self.yv))
      # 1st order positive
      self.otf_3_0 = fft2(self.psf * ysh[0])
      yshf = np.abs(fft2(ysh[0]))
      sx, sy = np.unravel_index(yshf.argmax(), yshf.shape)
      if (sx < Nw/2):
          sx = sx
      ...
      self.eh = np.append(self.eh, temp)
      zsp = self.zerosuppression(sx, sy)
      self.otf_3_0 = self.otf_3_0 * zsp
      self.imgf_3_0 = fft2(self.separr[5] * ysh[0])
      # 1st order negative
      self.otf_3_1 = fft2(self.psf * ysh[1])
      yshf = np.abs(fft2(ysh[1]))
      sx, sy = np.unravel_index(yshf.argmax(), yshf.shape)
      if (sx < Nw/2):
          sx = sx
      ...
      temp = np.sqrt(sx ** 2 + sy ** 2) / (2 * self.radius)
      self.eh = np.append(self.eh, temp)
      self.otf_3_1 = self.otf_3_1 * zsp
      self.imgf_3_1 = fft2(self.separr[6] * ysh[1])
      
      
    def shift4(self, angle, spacing):
        ''' shift data in freq space by multiplication in real space '''
        Nw = self.nx
        dx = self.dx
        kx = dx * np.cos(angle) / spacing
        ...
        ysh[1, :, :] = np.exp(2j * np.pi * (-kx * self.xv - ky * self.yv))
        # 1st order positive
        self.otf_4_0 = fft2(self.psf * ysh[0])
        yshf = np.abs(fft2(ysh[0]))
        sx, sy = np.unravel_index(yshf.argmax(), yshf.shape)
        if (sx < Nw/2):
            sx = sx
        ...
        self.eh = np.append(self.eh, temp)
        zsp = self.zerosuppression(sx, sy)
        self.otf_4_0 = self.otf_4_0 * zsp
        self.imgf_4_0 = fft2(self.separr[7] * ysh[0])
        # 1st order negative
        self.otf_4_1 = fft2(self.psf * ysh[1])
        yshf = np.abs(fft2(ysh[1]))
        sx, sy = np.unravel_index(yshf.argmax(), yshf.shape)
        if (sx < Nw/2):
            sx = sx
        ...
        temp = np.sqrt(sx ** 2 + sy ** 2) / (2 * self.radius)
        self.eh = np.append(self.eh, temp)
        self.otf_4_1 = self.otf_4_1 * zsp
        self.imgf_4_1 = fft2(self.separr[8] * ysh[1])
    ...
    def getoverlap3(self, angle, spacing, order, verbose=False):
        ''' shift 2nd order data '''
        dx = self.dx
        Nw = self.nx
        kx = dx * np.cos(angle) / spacing
    ...
    def getoverlap4(self, angle, spacing, order, verbose=False):
        ''' shift 2nd order data '''
        dx = self.dx
        Nw = self.nx
        kx = dx * np.cos(angle) / spacing
    ...
    def mapoverlap3(self, angle, spacing, order, nps=10, r_ang=0.02, r_sp=0.008):
        d_ang = 2 * r_ang / nps
        d_sp = 2 * r_sp / nps
        ang_iter = np.arange(-r_ang, r_ang + d_ang / 2, d_ang) + angle
        sp_iter = np.arange(-r_sp, r_sp + d_sp / 2, d_sp) + spacing
        ...
        for m, ang in enumerate(ang_iter):
            for n, sp in enumerate(sp_iter):
                print(m, n)
                mag, phase = self.getoverlap3(ang, sp, order)
                if np.isnan(mag):
                    magarr[m, n] = 0.0
                else:
                    magarr[m, n] = mag
    ...
    def mapoverlap4(self, angle, spacing, order, nps=10, r_ang=0.02, r_sp=0.008):
        d_ang = 2 * r_ang / nps
        d_sp = 2 * r_sp / nps
        ang_iter = np.arange(-r_ang, r_ang + d_ang / 2, d_ang) + angle
        sp_iter = np.arange(-r_sp, r_sp + d_sp / 2, d_sp) + spacing
        ...
        for m, ang in enumerate(ang_iter):
            for n, sp in enumerate(sp_iter):
                print(m, n)
                mag, phase = self.getoverlap4(ang, sp, order)
                if np.isnan(mag):
                    magarr[m, n] = 0.0
                else:
                    magarr[m, n] = mag
    ...
    def recon_1order(self, n, ang, spacing, phase, mag, path1, path2, zero_order=True):
        # construct n angles
        nx = self.nx
        ny = self.ny
        mu = self.mu
        ...
        for i in range(n):
            self.separate(i)
            self.shift0()
            self.shift1(ang[3*i], spacing[3*i])
            self.shift2(ang[3*i+1], spacing[3*i+1])
            ph1 = mag[3*i] * np.exp(1j * phase[3*i])
            ph2 = mag[3*i+1] * np.exp(1j * phase[3*i+1])
            # +1st order
            imgf = self.imgf_1_0
            otf = self.otf_1_0
            self.Snum += ph1 * otf.conj() * imgf
        ...
        if self.farme_order == 1:
            if zero_order == True:
                tf.imsave(self.path + '/' + path1 + '/' + 'final_image_withzero_1order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path1 + '/' + 'effective_OTF_withzero_1order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
            elif zero_order == False:
                tf.imsave(self.path + '/' + path1 + '/' +'final_image_nozero_1order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path1 + '/' + 'effective_OTF_nozero_1order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
        elif self.farme_order == 2:
            if zero_order == True:
                tf.imsave(self.path + '/' + path2 + '/' +'final_image_withzero_1order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path2 + '/' + 'effective_OTF_withzero_1order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
            elif zero_order == False:
                tf.imsave(self.path + '/' + path2 + '/' +'final_image_nozero_1order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path2 + '/' + 'effective_OTF_nozero_1order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')



    def recon_2order(self, n, ang, spacing, phase, mag, path1, path2, zero_order=True):
        # construct n angles
        nx = self.nx
        ny = self.ny
        mu = self.mu
        ...
        for i in range(n):
            ...
            self.shift1(ang[3*i], spacing[3*i])
            self.shift2(ang[3*i+1], spacing[3*i+1])
            self.shift3(ang[3*i+2], spacing[3*i+2])
            ph1 = mag[3*i] * np.exp(1j * phase[3*i])
            ph2 = mag[3*i+1] * np.exp(1j * phase[3*i+1])
            ph3 = mag[3*i+2] * np.exp(1j * phase[3*i+2])
            # +1st order
            imgf = self.imgf_1_0
            otf = self.otf_1_0
            self.Snum += ph1 * otf.conj() * imgf
            ...
            imgf = self.imgf_3_0
            otf = self.otf_3_0
            self.Snum += ph3 * otf.conj() * imgf
            self.Sden += abs(otf) ** 2
            # -3nd order
            imgf = self.imgf_3_1
            otf = self.otf_3_1
            self.Snum += ph3.conj() * otf.conj() * imgf
            self.Sden += abs(otf) ** 2
            # # finish
            # self.S = self.Snum/self.Sden
            # self.finalimage = np.fft.fftshift(ifft2(S))
        ...
        if self.farme_order == 1:
            if zero_order == True:
                tf.imsave(self.path + '/' + path1 + '/' + 'final_image_withzero_2order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path1 + '/' + 'effective_OTF_withzero_2order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
            elif zero_order == False:
                tf.imsave(self.path + '/' + path1 + '/' +'final_image_nozero_2order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path1 + '/' + 'effective_OTF_nozero_2order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
        elif self.farme_order == 2:
            if zero_order == True:
                tf.imsave(self.path + '/' + path2 + '/' +'final_image_withzero_2order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path2 + '/' + 'effective_OTF_withzero_2order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
            elif zero_order == False:
                tf.imsave(self.path + '/' + path2 + '/' +'final_image_nozero_2order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path2 + '/' + 'effective_OTF_nozero_2order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')

    
    def recon_3order(self, n, ang, spacing, phase, mag, path1, path2, zero_order=True):
        # construct n angles
        nx = self.nx
        ny = self.ny
        mu = self.mu
        ...
        for i in range(n):
            ...
            self.shift2(ang[3*i+1], spacing[3*i+1])
            self.shift3(ang[3*i+2], spacing[3*i+2])
            self.shift4(ang[3*i+3], spacing[3*i+3])
            ph1 = mag[3*i] * np.exp(1j * phase[3*i])
            ph2 = mag[3*i+1] * np.exp(1j * phase[3*i+1])
            ph3 = mag[3*i+2] * np.exp(1j * phase[3*i+2])
            ph4 = mag[3*i+3] * np.exp(1j * phase[3*i+3])
            # +1st order
            imgf = self.imgf_1_0
            otf = self.otf_1_0
            self.Snum += ph1 * otf.conj() * imgf
            ...
            imgf = self.imgf_4_0
            otf = self.otf_4_0
            self.Snum += ph4 * otf.conj() * imgf
            self.Sden += abs(otf) ** 2
            # -4nd order
            imgf = self.imgf_4_1
            otf = self.otf_4_1
            self.Snum += ph4.conj() * otf.conj() * imgf
            self.Sden += abs(otf) ** 2
            # # finish
            # self.S = self.Snum/self.Sden
            # self.finalimage = np.fft.fftshift(ifft2(S))
        ...
        if self.farme_order == 1:
            if zero_order == True:
                tf.imsave(self.path + '/' + path1 + '/' + 'final_image_withzero_3order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path1 + '/' + 'effective_OTF_withzero_3order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
            elif zero_order == False:
                tf.imsave(self.path + '/' + path1 + '/' +'final_image_nozero_3order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path1 + '/' + 'effective_OTF_nozero_3order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
        elif self.farme_order == 2:
            if zero_order == True:
                tf.imsave(self.path + '/' + path2 + '/' +'final_image_withzero_3order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path2 + '/' + 'effective_OTF_withzero_3order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')
            elif zero_order == False:
                tf.imsave(self.path + '/' + path2 + '/' +'final_image_nozero_3order.tif', self.finalimage.real.astype(np.float32), photometric='minisblack')
                tf.imsave(self.path + '/' + path2 + '/' + 'effective_OTF_nozero_3order.tif', np.abs(fftshift(self.S)).astype(np.float32), photometric='minisblack')

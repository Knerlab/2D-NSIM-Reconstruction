...
class si2D(object):
    ...
    # def sepmatrix(self):                                             # for 40 px 7phs
    #     nphases = self.nph
    #     norders = int((self.norders + 1) / 2)
    #     sepmat = np.zeros((self.norders, nphases), dtype=np.float32)
    #     for j in range(nphases):
    #         sepmat[0, j] = 1.0
    #         for order in range(1, norders):
    #             sepmat[2 * order - 1, j] = 2 * np.cos( 2 * np.pi * (j * order) / (nphases+0.35)) / (nphases+0.35)
    #             sepmat[2 * order, j] = 2 * np.sin(2 * np.pi * (j * order) / (nphases+0.35) ) / (nphases+0.35)
    #     # return np.linalg.inv(np.transpose(sepmat))
    #     # return sepmat/nphases
    #     return np.linalg.pinv(np.transpose(sepmat))
    
    def sepmatrix(self):                                             # for 15 px 7phs, 1 phs shift 1px
        nphases = self.nph
        norders = int((self.norders + 1) / 2)
        sepmat = np.zeros((self.norders, nphases), dtype=np.float32)
        for j in range(nphases):
    ...
    # def sepmatrix(self):                                             # for 20 px 10phs, 1 phs shift 1px
    #     nphases = self.nph
    #     norders = int((self.norders + 1) / 2)
    #     sepmat = np.zeros((self.norders, nphases), dtype=np.float32)
    #     for j in range(nphases):
